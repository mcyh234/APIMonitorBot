...
项目包含 FastAPI 后端、React WebUI、SQLite 数据库、OneBot HTTP/WS 接入、管理员命令系统、多轮添加配置流程、APIKey 加密存储、状态条图、网页快照推送和 Sub2API 渠道倍率监控。
...
- Sub2API 每分钟检测渠道分组 `rate_multiplier`，价格变化时发送高亮图片。
...
- `/price`：发送当前群聊或私聊通知对象绑定的 Sub2API 渠道倍率图片。
- `/cancel`：取消当前多轮对话。

普通用户调用 `/check`、`/status`、`/stat`、`/price` 有 5 分钟冷却；管理员不受冷却限制。
...
- 渠道接口：`GET {BaseURL}/api/v1/channels/available`。
- 记录每个 `platforms[].groups[]` 的 `rate_multiplier`。
- 首次看到某个分组和后续每次倍率变化都会写入倍率历史。
- 首次添加只保存当前倍率，不发送变动通知。
- 后续定时检测发现倍率变化时，发送价格变动图片。
- 图片中 Anthropic 渠道使用橙色，OpenAI 渠道使用绿色，发生变化的分组高亮。
- `/price` 图片会展开显示所有分组，包含当前倍率大字、涨跌百分比和按日期排列的历史折线；上涨为红色，下跌为绿色。
...
- 系统概览指标。
- API 配置表。
- Sub2API 渠道倍率面板，默认收起，点击后展开查看当前倍率和历史折线。
- 新增 API 表单。
- 管理员 QQ 列表。
- 最近发送失败原因。
- 最近接收到的 OneBot 消息。
