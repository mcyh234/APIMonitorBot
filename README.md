...
项目包含 FastAPI 后端、React WebUI、SQLite 数据库、OneBot WebSocket 接入、管理员命令系统、多轮添加配置流程、APIKey 加密存储、状态条图、网页快照推送和 Sub2API 渠道倍率监控。
...
WebUI 的“命令开关”可以开启或关闭 `/addapi`、`/addsub2`、`/list`、`/remove`、`/check`、`/status`、`/stat`、`/price`。`/cancel` 始终保留。
